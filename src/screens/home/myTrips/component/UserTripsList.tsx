  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} className="flex-1 mt-6">
      ...
      <View className="mb-6">
        <Image
          source={
            userTrips[0]?.tripRequestData?.placeImagUrl !== ''
              ? {uri: userTrips[0]?.tripRequestData?.placeImagUrl}
              : require('./../../../../assets/images/placeholder_image.jpg')
          }
          className="w-full h-44 object-cover rounded-3xl"
        />
        <Text
          className=" pt-4 pl-2 text-xl text-black"
          style={{fontFamily: 'Outfit-Medium'}}>
          {userTrips[0]?.tripRequestData?.locationInfo?.name}
        </Text>
        <View className="flex-row w-full px-2 justify-between">
          <Text
          ...
          <Text
            className="text-lg text-black"
            style={{fontFamily: 'Outfit-Regular'}}>
            {userTrips[0]?.tripRequestData?.budgetInfo?.title}
          </Text>
        </View>
        <TouchableOpacity
          activeOpacity={0.6}
      ...
      {userTrips.length > 1 && (
        <FlatList
          ...
          renderItem={({item}) => (
            <UsertripCard trip={item} navigation={navigation} />
          )}
          scrollEnabled={false} // Vertical scrolling
        />
      )}
    </ScrollView>
  );
